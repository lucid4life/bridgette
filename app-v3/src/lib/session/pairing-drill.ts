// "Pour the room" — the call-the-pairing say-it drill behind /pour. Same item
// model as the path (it grades the SAME pairing:<foodId> items Stage 5 tracks;
// "pour" is a presentation MODE, not a new card set) and the same mechanics as
// review.ts/romance.ts/build.ts at ONE forced rung 'pour': every dish is asked
...
// SAME pour rung ~3 steps later until cleared. Full UI contract: ./types.ts.
...
export function createPairingDrillSession(
  ...
  const queue: QueueEntry[] = items.map((item) => {
    if (item.kind !== 'pairing')
      throw new Error(
        `session: pour item '${item.id}' is not a dish pairing — only pairing:* items get poured`
      );
    return { kind: 'retrieve', item, retry: false };
  });

  const clearedList: PourItemResult[] = []; // in clearing order
  ...
  function current(): Step | null {
    ...
    return head.kind === 'reteach'
      ...
      : { type: 'quiz', item: head.item, rung: 'pour' };
  }

  function answerMc(_choiceIndex: number): McAnswer {
    throw new Error('session: pour sessions have no MC steps');
  }
  ...
  function selfGrade(gotIt: boolean): void {
    ...
    if (gotIt) {
      // One correct call clears the item. A retry clears SILENTLY — the
    ...
    // Retry ~3 steps out at the SAME pour rung, with the teach screen NEXT.
  ...
  function progress(): PourProgress {
    return {
      position,
