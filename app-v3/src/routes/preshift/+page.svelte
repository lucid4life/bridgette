  ...
  {:else if step && prog}
    ...
    {#key prog.position}
      {#if step.type === 'quiz' && step.rung === 'cued'}
        ...
        <FlashReveal prompt={c.prompt} hint={c.hint} answer={c.answer} kicker="shaky call — with a hint" ongrade={grade} note="honest call — misses come back tonight, not on the floor" />
      {:else if step.type === 'quiz'}
        {@const f = freeFor(step.item)}
        <FlashReveal prompt={f.prompt} answer={f.answer} detail={f.detail} kicker="shaky call — cold" ongrade={grade} note="honest call — misses come back tonight, not on the floor" />
      {:else}
